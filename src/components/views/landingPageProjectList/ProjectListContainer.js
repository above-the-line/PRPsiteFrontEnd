...
import SpeedDialClass from "./../../speedDialer/SpeedDialClass";
...
const FILTERED_GQL_QUERY = gql`
  query ArbitraryNameForfilterProjectsByRole(
    $rolesOfInterest: Avi_RolesCreateInput
  ) {
...
class ProjectListContainer extends Component {
  ...
  executeSearch = async () => {
    ...
    this.state.project_properties_to_render.map(
      (role) => (gql_vars["rolesOfInterest"][role] = true)
    );
    ...
    return (
      <div className={styles.fullScreenContainer}>
        ...
        {/* <SearchOptionsSpeedDailer checkBoxOptions = {this.state.available_project_properties} 
                ...
                /> */}
        <SpeedDialClass
          container_function_checkbox_click_handler={
            this.checkbox_click_handler
          }
        />
      </div>
    );
